...
/**
 * In-memory write-through cache in front of a backing {@link Store}. Loads everything at startup
 * and serves reads synchronously from memory, so main-thread {@code loadProtection().join()}
 * calls during interactions never block on the database. Writes update the cache and are
 * forwarded (buffered/async) to the backing store. Ported from the Bukkit implementation with
 * bStats metrics dropped.
 */
public class SimpleProtectionCache implements Store {
    ...
    public CompletableFuture<EntityProtection> loadEntityProtection(UUID id) {
        return CompletableFuture.completedFuture(cachedEntities.get(id));
    }
